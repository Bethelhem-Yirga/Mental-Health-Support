  ...
  return (
    <div className="h-[calc(100vh-12rem)] transition-all duration-300">
      <div className="flex h-full gap-4">
        ...
        {/* Back Button - Mobile */}
        <Link href="/" className="lg:hidden fixed top-24 left-4 z-20 bg-white dark:bg-gray-800 p-2 rounded-full shadow-lg">
        ...
        <div className={`flex-1 flex flex-col rounded-2xl shadow-2xl overflow-hidden ${darkMode ? 'bg-gray-900/95' : 'bg-white/95'} backdrop-blur-sm`}>
                    ...
                    <div key={msg.id || idx} className={`flex mb-4 ${isOwn ? 'justify-end' : 'justify-start'} animate-slideIn`}>
                      {!isOwn && (
                        <div className="flex-shrink-0 mr-3">
                          <div className={`w-10 h-10 bg-gradient-to-r ${getAvatarGradient(msg.anonymousId)} rounded-xl flex items-center justify-center text-white text-sm font-bold shadow-lg`}>
                      ...
                      <div className={`max-w-[70%] ${isOwn ? 'items-end' : 'items-start'}`}>
                        ...
                        <div className={`relative p-3 rounded-2xl shadow-lg transition-all hover:scale-[1.02] ${
                          isOwn
                      ...
                      {isOwn && (
                        <div className="flex-shrink-0 ml-3">
                          <div className="w-10 h-10 bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl flex items-center justify-center text-white text-sm font-bold shadow-lg">YOU</div>
                        </div>
                      )}
                    </div>
                  )
          ...
          <div className={`p-4 border-t ${darkMode ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-white'}`}>
            <form onSubmit={handleSendMessage} className="flex items-end gap-2">
              <div className="flex-1 relative">
                ...
                <div className="absolute right-2 bottom-2 flex items-center gap-1">
                  <button type="button" onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="p-1.5 text-gray-400 hover:text-gray-600 rounded-full transition">
                    <Smile className="w-5 h-5" />
                  </button>
                  <button type="button" className="p-1.5 text-gray-400 hover:text-gray-600 rounded-full transition">
            ...
            {showEmojiPicker && (
              <div className="absolute bottom-24 left-6 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-3 border animate-slideUp z-10">
                <div className="grid grid-cols-8 gap-2">
                  {emojis.map(emoji => (
                    <button key={emoji} onClick={() => addEmoji(emoji)} className="text-2xl hover:bg-gray-100 p-2 rounded-lg transition transform hover:scale-110">
                      {emoji}
                    </button>
                  ))}
                </div>
      ...
      <div className="hidden lg:block fixed bottom-6 right-6">
        <div className="bg-white dark:bg-gray-800 rounded-full shadow-lg p-2 flex gap-2">
          <button onClick={() => setDarkMode(!darkMode)} className="p-2 hover:bg-gray-100 rounded-full transition">
            {darkMode ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </button>
          <button onClick={() => setSoundEnabled(!soundEnabled)} className="p-2 hover:bg-gray-100 rounded-full transition">
